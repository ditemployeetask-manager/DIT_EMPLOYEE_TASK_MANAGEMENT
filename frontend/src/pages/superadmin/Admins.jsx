...
const Admins = () => {
  ...
  // Selected Admin for edit/details
  ...
  const fetchData = async () => {
    ...
    try {
      const [adminsRes, groupsRes] = await Promise.all([
        adminService.getAllAdmins(page, limit, search),
      ...
      if (adminsRes.success) {
        ...
        setTotalRecords(adminsRes.totalRecords || adminsRes.data.length);
      } else {
      ...
      console.error(err);
      setError("An error occurred while loading admins and groups data.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreateSubmit = async (e) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setCredentials({
          employeeId: res.data.adminId || res.data.admin?.employee_id,
  ...
  const handleEditOpen = (admin) => {
    ...
    setFormData({
      ...
      groupIds: admin.groups ? admin.groups.map(g => g.id) : [],
    });
    setFormErrors({});
    setIsEditOpen(true);
  };
  ...
  const handleEditSubmit = async (e) => {
    ...
    setError("");
    try {
      const res = await adminService.updateAdmin(selectedAdmin.id, formData);
      if (res.success) {
  ...
  const handleStatusToggle = async (admin) => {
    try {
      ...
      const res = await adminService.updateAdminStatus(admin.id, newStatus);
      if (res.success) {
        setSuccess(`Admin status updated to ${newStatus ? "Active" : "Inactive"}!`);
  ...
  const handleResetPassword = async (admin) => {
    if (!window.confirm(`Are you sure you want to reset the password for admin ${admin.name}?`)) return;
    
    setError("");
    try {
      const res = await adminService.getAdminById(admin.id); // Check or execute reset
      // The resetPassword endpoint is: PUT /api/users/:id/reset-password
      // Let's verify if resetPassword is in employeeService or userService (yes, the backend routes has it: PUT /api/users/:id/reset-password, which works for all employee/admins).
      // Let's import and use the service or define the resetPassword request.
      // Wait, employeeService has resetPassword! We can import it, or we can just fetch via API. Let's see:
      const resPass = await employeeService.resetPassword(admin.id);
      if (resPass.success) {
        setCredentials({
          employeeId: admin.employee_id,
          tempPassword: resPass.data.temporaryPassword,
          action: "reset"
        });
  ...
  const activeAdminsCount = admins.filter(a => a.status).length;
  ...
  return (
    <div className="select-none text-left py-2 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-800 tracking-tight">Admins Management</h2>
          <p className="text-xs text-slate-500 font-semibold mt-1">
            Provision administration accounts, assign department scopes, and monitor admin activity.
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
        ...
        <div className="bg-white border border-slate-100 p-5 rounded-2xl shadow-sm flex items-center gap-4">
          ...
          <div>
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Active Admins</span>
            <h4 className="text-xl font-extrabold text-slate-800 mt-0.5">{activeAdminsCount} / {admins.length}</h4>
          </div>
        </div>
        ...
        {loading ? (
          <div className="flex items-center justify-center p-20">
            <div className="flex flex-col items-center gap-3">
              ...
              <span className="text-xs font-bold text-slate-500 animate-pulse">Loading accounts...</span>
            </div>
          </div>
        ) : admins.length === 0 ? (
          <div className="p-20 text-center flex flex-col items-center justify-center">
            ...
            <h4 className="text-sm font-bold text-slate-800">No Admins Found</h4>
            <p className="text-xs text-slate-400 font-semibold mt-1">Try searching for something else or add a new admin account.</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 text-xs font-semibold text-slate-700">
                {admins.map((admin) => (
                  <tr key={admin.id} className="hover:bg-slate-50/30">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col min-w-0">
                          <span className="font-bold text-slate-800 text-sm truncate">{admin.name}</span>
                          <span className="text-[10px] text-slate-400 font-bold">{admin.employee_id}</span>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        <span className="text-slate-800 truncate">{admin.email}</span>
                        <span className="text-[10px] text-slate-400 font-semibold mt-0.5">{admin.phone}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white border border-slate-100 rounded-2xl shadow-xl w-full max-w-lg p-7 text-left space-y-6">
            <div className="flex justify-between items-center pb-3 border-b border-slate-100">
              <h3 className="text-lg font-bold text-slate-800">Add New Admin Account</h3>
      ...
      {isEditOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white border border-slate-100 rounded-2xl shadow-xl w-full max-w-lg p-7 text-left space-y-6">
            <div className="flex justify-between items-center pb-3 border-b border-slate-100">
              <h3 className="text-lg font-bold text-slate-800">Edit Admin Settings</h3>
      ...
      {isDetailOpen && selectedAdmin && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white border border-slate-100 rounded-2xl shadow-xl w-full max-w-md p-7 text-left space-y-6">
            <div className="flex justify-between items-center pb-3 border-b border-slate-100">
              <h3 className="text-lg font-bold text-slate-800">Admin Account Profile</h3>
            ...
            <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-xl border border-slate-100">
              ...
              <div className="flex-grow min-w-0">
                <h4 className="text-sm font-black text-slate-800 truncate">{selectedAdmin.name}</h4>
                <span className="text-[10px] text-slate-400 font-bold">{selectedAdmin.employee_id}</span>
              </div>
            ...
            <div className="space-y-4 text-xs font-semibold text-slate-600">
              <div className="flex justify-between border-b border-slate-50 pb-2">
                ...
                <span className="text-slate-800">{selectedAdmin.designation}</span>
              </div>
              <div className="flex justify-between border-b border-slate-50 pb-2">
                <span className="text-slate-400">EMAIL ADDRESS</span>
                <span className="text-slate-800 truncate max-w-[200px]">{selectedAdmin.email}</span>
              </div>
              <div className="flex justify-between border-b border-slate-50 pb-2">
                <span className="text-slate-400">PHONE NUMBER</span>
                <span className="text-slate-800">{selectedAdmin.phone}</span>
              </div>
              <div className="flex flex-col gap-1.5 border-b border-slate-50 pb-3">
                ...
                <div className="flex flex-wrap gap-1 mt-1">
                  {selectedAdmin.groups && selectedAdmin.groups.length > 0 ? (
                    selectedAdmin.groups.map(g => (
          ...
          <div className="bg-white border border-slate-100 rounded-2xl shadow-xl w-full max-w-sm p-7 text-left space-y-5 animate-scale-in">
            <div className="text-center space-y-2">
              ...
              <h3 className="text-lg font-black text-slate-800">
                {credentials.action === "created" ? "Admin Created Successfully!" : "Password Reset Successfully!"}
              </h3>
              <p className="text-[11px] text-slate-400 font-semibold">
                Please copy and share these temporary credentials with the admin immediately.
              </p>
            </div>
            ...
            <div className="bg-slate-50 border border-slate-100 rounded-xl p-4.5 space-y-3.5 text-xs font-semibold">
              <div className="flex justify-between items-center">
                <div className="flex flex-col">
                  <span className="text-[10px] text-slate-400 font-bold uppercase">Admin / Employee ID</span>
