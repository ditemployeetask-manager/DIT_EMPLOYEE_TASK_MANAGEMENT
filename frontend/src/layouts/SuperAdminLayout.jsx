...
const SuperAdminLayout = () => {
  ...
  const getPageTitle = (pathname) => {
    if (pathname.endsWith("/dashboard")) return "Super Admin Dashboard";
    if (pathname.endsWith("/employees")) return "Employees Management";
    if (pathname.endsWith("/admins")) return "Admins Management";
    if (pathname.endsWith("/groups")) return "Groups / Departments";
    if (pathname.endsWith("/audit-logs")) return "Audit Logs";

    if (pathname.includes("/employees/")) return "Employee Profile Details";
    if (pathname.includes("/admins/")) return "Admin Monitoring Details";
    if (pathname.includes("/notifications/")) return "Notification Details";
    if (pathname.endsWith("/notifications")) return "Notifications";
    if (pathname.endsWith("/send-notification")) return "Send Notification";
    if (pathname.endsWith("/analytics")) return "Reports & Analytics";
    if (pathname.endsWith("/export")) return "Export Reports";
    if (pathname.endsWith("/change-password")) return "Security Settings";
    return "Super Admin Dashboard";
  };

  // Nav items with groups matching mockup
  const navGroups = [
    {
      items: [
        {
    ...
    },
    {
      ...
      items: [
        {
          path: "/superadmin/send-notification",
  ...
  return (
    <div className="flex h-screen bg-[#f8fafc] overflow-hidden">
      ...
      <aside className={`bg-white border-r border-slate-200 flex flex-col justify-between transition-all duration-300 ${collapsed ? "w-20" : "w-64"} flex-shrink-0 relative`}>
        ...
        {/* Sidebar Nav Items */}
        <div className="flex-grow overflow-y-auto px-3 py-4 space-y-4 select-none">
          {navGroups.map((group, idx) => (
            <div key={idx} className="space-y-1">
              {group.title && !collapsed && (
                <div className="px-3 text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1 text-left">
                  {group.title}
                </div>
              )}
              {group.items.map((item) => (
                <NavLink
                  key={item.path}
                  to={isMandatory ? "/superadmin/change-password" : item.path}
                  onClick={(e) => {
                    if (isMandatory && item.path !== "/superadmin/change-password") {
                      e.preventDefault();
                    }
                  }}
                  className={({ isActive }) =>
                    `flex items-center justify-between p-2.5 rounded-lg text-sm font-semibold transition-all ${
                      isMandatory && item.path !== "/superadmin/change-password" ? "opacity-50 cursor-not-allowed text-slate-400" : ""
                    } ${
                      isActive && (!isMandatory || item.path === "/superadmin/change-password")
        ...
        <div className="p-3 border-t border-slate-100 bg-white">
          ...
          {!collapsed && (
            <div className="bg-blue-50/50 border border-blue-100 rounded-xl p-3.5 flex flex-col items-start text-left shadow-sm">
              ...
              <p className="text-[10px] text-slate-500 font-medium mt-1 leading-relaxed">
                Building a better digital workplace for a better tomorrow.
              </p>
            </div>
          )}
        </div>
      ...
      <div className="flex flex-col flex-grow h-screen overflow-hidden">
        ...
        <header className="bg-white border-b border-slate-200 h-20 px-6 flex items-center justify-between select-none">
          ...
          <div className="flex items-center gap-4 relative">
            ...
            <button 
              onClick={() => !isMandatory && navigate("/superadmin/notifications")}
              disabled={isMandatory}
            ...
            <div className="relative">
              ...
              >
                ...
                <div className="hidden lg:flex flex-col text-left leading-normal">
                  <span className="text-xs font-bold text-slate-800">{user?.name || "Super Admin"}</span>
                  <span className="text-[10px] text-slate-400 font-semibold">{user?.employee_id || "SA-001"} • System Admin</span>
                </div>
                {!isMandatory && (
              ...
              {showProfileDropdown && !isMandatory && (
                <div className="absolute right-0 mt-2 w-48 bg-white border border-slate-150 rounded-xl shadow-lg py-1.5 z-50 animate-fade-in text-left">
                  ...
                  <button 
                    onClick={() => { setShowProfileDropdown(false); navigate("/superadmin/change-password"); }}
                    className="w-full text-left px-4 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50 hover:text-slate-900 transition-colors"
                  >
                    Change Password
                  </button>
